...
import { coastStats } from '@/data/coatesvilleData';
...
export default function Coatesville() {
  const navigate = useNavigate();
  const [distId] = useState<'half'>('half');

  const seedCRM = useMemo(() => Math.min(...coastStats.map(s => s.winnerM)) + 1, []);
  const seedCRW = useMemo(() => Math.min(...coastStats.map(s => s.winnerW)) + 1, []);

  const recordM = { time: '1:13:31', holder: 'Jonathan Jackson', nationality: 'NZL', club: '—', year: 2019, previous: '—' };
  const recordW = { time: '1:20:10', holder: 'Katrina Andrew',   nationality: 'NZL', club: '—', year: 2024, previous: '—' };
  ...
  return (
    <main>
      ...
      <section style={{ padding: '48px 0 32px', borderBottom: '0.5px solid var(--rule)' }}>
        <div className="page">
          <div className="eyebrow mb-24">Road · Auckland · Coatesville</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 48, alignItems: 'end' }} className="race-head-grid">
            <div>
              ...
              <div className="flex gap-8 mt-20" style={{ flexWrap: 'wrap' }}>
                <button className={`pill ${distId === 'half' ? 'active' : ''}`}>21.1 km</button>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, fontSize: 12 }}>
              ...
              <div>
                ...
                <div><a href="https://coatesville.werun.nz/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--ink)', textDecoration: 'underline', textUnderlineOffset: 4 }}>coatesville.werun.nz ↗</a></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 48, alignItems: 'start' }} className="overview-grid">
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, borderTop: '0.5px solid var(--rule)', borderBottom: '0.5px solid var(--rule)' }}>
                {[
                  { label: 'Climb',   val: '~250 m', sub: '↑ cumulative' },
                  { label: 'Descent', val: '~250 m', sub: '↓ cumulative' },
              ...
              <div className="mt-24" style={{ fontSize: 12, lineHeight: 1.6 }}>
                ...
                <div>Sealed road and gravel trail</div>
                <div className="label mt-16 mb-8">Character</div>
                <div>A challenging rural loop through the Coatesville countryside north of Auckland. Multiple significant climbs reward runners with sweeping views across the Rodney farmland. One of Auckland's hilliest and most scenic half marathons, held annually since 2011.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="section">
        <div className="page">
          ...
          <AveragesChart stats={coastStats} />
        </div>
      </section>
          ...
          <div className="card-dark">
            ...
            <div style={{ color: 'var(--on-dark)', marginTop: 40, paddingTop: 32, borderTop: '0.5px solid var(--on-dark-rule)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
              <div>
                ...
                <CRWinnerChart stats={coastStats} gender="men" seedCR={seedCRM} />
              </div>
              <div>
                <div className="label mb-16" style={{ color: 'var(--on-dark-meta)' }}>Women · winner vs CR</div>
                <CRWinnerChart stats={coastStats} gender="women" seedCR={seedCRW} />
              </div>
            </div>
          </div>
        </div>
