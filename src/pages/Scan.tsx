...
// NOTE: This page mirrors the requested UI/logic.
// Firebase is not configured in this project yet.
// For now, we persist to localStorage as a safe fallback.
// Once Firebase is added, replace `saveLog` with the Firestore code
// and import your initialized `db`.
...
type LogEntry = {
  ...
  notes: string;
  passFail: "pass" | "fail";
  createdAt: number; // epoch ms for local fallback
};

// Local, simple persistence while Firebase isn’t wired.
...
export default function ScanPage() {
  ...
  const padNbsp = (s: string, len: number) => s + "\u00A0".repeat(Math.max(0, len - s.length));
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // Local fallback persistence. Replace with Firestore when ready:
      ...
      setNotes("");
      setPassFail("pass");
      const nameEl = document.getElementById("name-input") as HTMLInputElement | null;
