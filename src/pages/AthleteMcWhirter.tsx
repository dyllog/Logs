      ...
      <section style={{ background: 'var(--surface-dark)', color: 'var(--on-dark)', padding: '48px 0 40px', borderBottom: '0.5px solid var(--on-dark-rule)' }}>
        <div className="page">
          ...
          <div style={{ marginTop: 32, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button className="btn" style={{ color: 'var(--on-dark)', borderColor: 'var(--on-dark)', fontSize: 10.5 }}
                    onClick={() => navigate('/athletes/blair-mcwhirter/report')}>
              View Report →
            </button>
          </div>
        </div>
                ...
                {sortedResults.map((r, i) => (
                  <tr key={i} className="row"
                      onClick={() => {
                        ...
                        else if (r.race === 'Christchurch Marathon' || r.race === 'Christchurch Half') navigate(`/races/christchurch-marathon?year=${r.year}&dist=${d}`);
