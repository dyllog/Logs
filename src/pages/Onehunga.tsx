...
import { onehungaHalfStats, onehunga10kStats } from '@/data/onehungaData';
...
export default function Onehunga() {
  const navigate = useNavigate();
  const [distId, setDistId] = useState<'half' | '10k'>('half');

  const seedCRM    = useMemo(() => Math.min(...onehungaHalfStats.map(s => s.winnerM)) + 1, []);
  const seedCRW    = useMemo(() => Math.min(...onehungaHalfStats.map(s => s.winnerW)) + 1, []);
  const seedCRM10k = useMemo(() => Math.min(...onehunga10kStats.map(s => s.winnerM)) + 1, []);
  const seedCRW10k = useMemo(() => Math.min(...onehunga10kStats.map(s => s.winnerW)) + 1, []);

  const recordM    = { time: '1:13:07', holder: 'David Atkinson',  nationality: 'NZL', club: '—', year: 2025, previous: '—' };
  const recordW    = { time: '1:24:02', holder: 'Jaye Atkin',      nationality: 'NZL', club: '—', year: 2023, previous: '—' };
  const recordM10k = { time: '33:30',   holder: 'Patrich Bravo',   nationality: 'NZL', club: '—', year: 2023, previous: '—' };
  const recordW10k = { time: '40:25',   holder: 'Katie Curd',      nationality: 'NZL', club: '—', year: 2023, previous: '—' };
  ...
  return (
    <main>
      <section style={{ padding: '48px 0 32px', borderBottom: '0.5px solid var(--rule)' }}>
        <div className="page">
          <div className="eyebrow mb-24">Road · Auckland · Onehunga</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 48, alignItems: 'end' }} className="race-head-grid">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, fontSize: 12 }}>
              <div><div className="label mb-8">Location</div><div>Onehunga, Auckland</div></div>
      ...
      <section id="results" className="section">
        <div className="page">
          <RaceResultsBlock
            ...
            raceId={isHalf ? 'onehunga-half' : 'onehunga-10k'}
        ...
        <div className="page">
          <div className="section-header">
            ...
            <div className="dimmed" style={{ fontSize: 12, maxWidth: 280, textAlign: 'right' }}>
              Road · Onehunga Harbour foreshore out & back
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 48, alignItems: 'start' }} className="overview-grid">
            ...
            <div>
              ...
              <div className="mt-24" style={{ fontSize: 12, lineHeight: 1.6 }}>
                ...
                <div>Sealed road · flat harbour foreshore</div>
                <div className="label mt-16 mb-8">Character</div>
                <div>{isHalf
                  ? 'A flat out-and-back along the Onehunga foreshore and Manukau Harbour coastline in South Auckland. One of the Run21 Auckland series events, offering fast conditions along the waterfront with views across the Manukau Harbour. Run annually since 2021.'
                  : 'A fast 10 km out-and-back along the Onehunga foreshore, ideal for personal bests. The flat sealed route follows the Manukau Harbour edge with the turnaround at the halfway mark.'
                }</div>
              </div>
            </div>
          </div>
      ...
      <section className="section">
        <div className="page">
          ...
          <AveragesChart stats={isHalf ? onehungaHalfStats : onehunga10kStats} />
        </div>
      </section>
          ...
          <div className="card-dark">
            ...
            <div style={{ color: 'var(--on-dark)', marginTop: 40, paddingTop: 32, borderTop: '0.5px solid var(--on-dark-rule)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
              <div>
                ...
                <CRWinnerChart stats={isHalf ? onehungaHalfStats : onehunga10kStats} gender="men" seedCR={isHalf ? seedCRM : seedCRM10k} />
              </div>
              <div>
                <div className="label mb-16" style={{ color: 'var(--on-dark-meta)' }}>Women · winner vs CR</div>
                <CRWinnerChart stats={isHalf ? onehungaHalfStats : onehunga10kStats} gender="women" seedCR={isHalf ? seedCRW : seedCRW10k} />
              </div>
            </div>
          </div>
        </div>
